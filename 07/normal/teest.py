...
def process_images_with_multiple_questions(folder_path, api_provider, api_key, questions, output_csv, selected_images=None, model_name=None):
    ...
    for question in questions:
        if question.get('type') == 'text':
            ...
            # Prepare data for scored file
            scored_data = []
            for idx, response in enumerate(text_responses[question_name], 1):
                scored_data.append({
                    'Index': idx,
                    'Response': response,
                    'Accuracy': '',
            ...
            if os.path.exists(scored_filename):
                ...
                # Get next index
                next_index = df_existing['Index'].max() + 1 if not df_existing.empty else 1
                # Update indices
                df_new['Index'] = range(next_index, next_index + len(df_new))
                # Append
                df_combined = pd.concat([df_existing, df_new], ignore_index=True)
...
if __name__ == "__main__":
    ...
    QUESTIONS = [
        {
            'name': 'Osteophyte_Formation',
            'type': 'binary',
            'prompt': 'You are now a senior expert in oral and maxillofacial radiology. Please analyze this TMJ CBCT scan according to the following anatomical sequence: Anterior surface of the condyle: Observe for any outgrowths (osteophytes). Superior surface of the condyle: Observe for any abnormal depressions or morphological irregularities (erosions). Medullary space: Observe whether there are radiolucent areas (cyst-like lesions/subchondral cysts) within the bone.Based on this analysis, determine the likelihood of pathological bony changes in the TMJ. If the probability is greater than 50%, answer \'yes\', otherwise answer \'no\'.',
            'keywords': ['yes']
        }
